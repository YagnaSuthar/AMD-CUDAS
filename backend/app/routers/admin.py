...
from app.models.auth import AuthUser, College, CollegeStatus, Company
from app.schemas.auth import AnalyticsResponse, CollegeResponse, MessageResponse
...
async def verify_college(
    ...
    college.status = CollegeStatus.APPROVED
...
async def get_analytics(
    ...
    approved_colleges = await db.scalar(
        select(func.count(College.id)).where(College.status == CollegeStatus.APPROVED)
    )
    pending_colleges = await db.scalar(
        select(func.count(College.id)).where(College.status == CollegeStatus.PENDING)
    )
    ...
    return AnalyticsResponse(
        ...
        approved_colleges=approved_colleges or 0,
        pending_colleges=pending_colleges or 0,
